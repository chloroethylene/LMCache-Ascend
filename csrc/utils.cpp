...
kvcache_ops::AscendType get_dtype_from_torch(at::ScalarType scalarType) {
  ...
  } else if (scalarType == at::ScalarType::Char ||
...
MultiLayerKVConfig prepare_multi_layer_kv_config(
  ...
  config.num_tokens_lmc_chunk = (key_value.dim() >= 3)
                                    ? static_cast<int>(key_value.size(2))
                                    : config.num_tokens;
  ...
  if (config.is_dsa_c8) {
    config.kv_size = 4;
    // C8 uses multi-plane transfer; hidden_dims stores per-plane K bytes for
    // compatibility.
...
void compute_multi_layer_ub_params(MultiLayerKVConfig &config,
  ...
  if (config.is_dsa_c8) {
    max_hidden_dims =
        std::max({config.k_hidden_dims, config.v_hidden_dims,
                  config.dsa_hidden_dims, config.dsa_c8_scale_plane_bytes});
