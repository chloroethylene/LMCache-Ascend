...
This module provides NPU-side KV cache management functionality, including:
- NPUCacheContext: Manages shape and pointers to vLLM NPU KV cache tensors
- Helper functions for tensor operations and key resolution

Adapted from lmcache.v1.multiprocess.gpu_context for CUDA.
"""
...
# lmcache_ascend c_ops for NPU kernel operations
...
def unwrap_kv_cache_tensors(kv_caches: KVCache) -> list[torch.Tensor]:
    ...
    Args:
        kv_caches: List of AscendIPCWrapper objects wrapping NPU tensors.

    Returns:
        List of unwrapped torch.Tensor objects on NPU.
    """
    unwrapped_tensors = []
...
def list_to_gpu_tensor(lis: list[int], device: torch.device) -> torch.Tensor:
    """
    Convert a list of integers to a GPU tensor.

    Args:
        lis: List of integer pointers.
        device: Target NPU device.

    Returns:
        torch.Tensor on NPU with the integer values.
    """
...
class NPUCacheContext:
    ...
    def __init__(
        ...
        # retains per-group NPU resources (pointer tensors, tmp buffers).
        ...
        # NPU streams - using torch.npu.Stream since cupy does not support NPU
        self.npu_stream_ = torch.npu.Stream(device=self.device_)
        self.cupy_stream_ = None  # cupy does not support NPU

        _, high_priority = torch.npu.Stream.priority_range()
        self.high_priority_npu_stream_ = torch.npu.Stream(
            device=self.device_, priority=high_priority
        )
        self.high_priority_cupy_stream_ = None  # cupy does not support NPU

        # Extra initialization - log directly without cupy stream
        logger.info("Initialized NPU stream on device %s", str(self.device_))
    ...
    def cupy_stream(self):
        """
        Returns None since cupy does not support NPU.
        """
    ...
    def get_tmp_gpu_buffer_flat(self, chunk_idx: int) -> torch.Tensor:
        """Returns the flat uint8 view of the temporary NPU buffer for the
    ...
    def get_tmp_chunk_gpu_buffer(self, group_idx: int = 0) -> torch.Tensor:
        """
        Returns a view of the temporary NPU buffer for the given group,
    ...
    def get_tmp_chunk_gpu_buffer_batched(
        ...
        pre-allocated temporary NPU buffer for the given group, each
    ...
    def stage_block_ids(self, block_ids: list[int]) -> torch.Tensor:
        """Copy block_ids into the pre-allocated NPU buffer and return a
        ...
        Returns:
            A NPU int64 tensor view into the pre-allocated buffer.
        """
        n = len(block_ids)
...
class PlainNPUCacheContext:
    """
    A plain NPU cache context that has a single contiguous 2LTD buffer.
    """

    def __init__(self, kv_caches: KVCache, lmcache_chunk_size: int = 256):
        assert len(kv_caches) == 1, (
            "PlainNPUCacheContext only supports a single KV cache tensor"
        )
        ...
        assert len(shape) == 4, "Expected [2, L, T, D] for plain NPU cache"
        ...
        # NPU streams - using torch.npu.Stream since cupy does not support NPU
        self._npu_stream = torch.npu.Stream(device=self._device)
        self._cupy_stream = None  # cupy does not support NPU

        _, high_priority = torch.npu.Stream.priority_range()
        self._high_priority_npu_stream = torch.npu.Stream(
            device=self._device, priority=high_priority
        )
        self._high_priority_cupy_stream = None  # cupy does not support NPU

        # Extra initialization - log directly without cupy stream
        logger.info("Initialized NPU stream on device %s", str(self._device))
    ...
    def get_tmp_gpu_buffer(self, num_tokens: int) -> torch.Tensor:
        """
        Returns the temporary NPU buffer for transfers.
        """
    ...
    @property
    def high_priority_stream(self) -> torch.npu.Stream:
        return self._high_priority_npu_stream
