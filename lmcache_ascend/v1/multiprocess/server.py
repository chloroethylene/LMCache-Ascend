...
from lmcache_ascend.v1.multiprocess.config import MPServerConfig as AscendMPServerConfig
...
def get_layout_desc(npu_context: NPUCacheContext, num_tokens: int) -> MemoryLayoutDesc:
    """Get the memory layout description for a given NPU context and number of tokens.

    Supports multiple KV layer groups with different shapes and dtypes.

    Args:
        npu_context: The NPU cache context containing the KV cache information.
    ...
    """
    num_groups = npu_context.kv_layer_groups_manager.num_groups
    shapes = [
        npu_context.get_kv_buffer_shape(num_tokens, group_idx)
        for group_idx in range(num_groups)
    ]
    dtypes = [
        npu_context.kv_layer_groups_manager.kv_layer_groups[group_idx].dtype
        for group_idx in range(num_groups)
    ]
...
class _PrefetchJob:
    ...
    # on the happy path; 0 for early-exit paths (no NPU context matches
...
# Main class for the NPU cache engine
class NPUCacheEngine:
    def __init__(
        self,
        ...
        # NPU ID -> NPU cache context
        self.npu_contexts: dict[int, NPUCacheContext] = {}

        # NPU ID -> (model name, world size) as metadata
        ...
        # the layout desc returned by the npu context is the same.
        self.npu_context_meta: dict[int, tuple[str, int]] = {}
    ...
    def register_kv_cache(
        ...
        Args:
            instance_id (int): The NPU instance ID (such as PID).
            ...
            engine_type: Which serving engine produced the caches.
                Forwarded to :class:`NPUCacheContext` for format detection.
            layout_hints: See :class:`LayoutHints`.  Forwarded to
                :class:`NPUCacheContext` for NPU KV format detection.
        """
        ...
        self.npu_contexts[instance_id] = npu_context
        self.npu_context_meta[instance_id] = (model_name, world_size)
        logger.info(
    ...
    def unregister_kv_cache(self, instance_id: int) -> None:
        ...
        Args:
            instance_id (int): The NPU instance ID (such as PID).
        """
        if instance_id in self.npu_contexts:
            del self.npu_contexts[instance_id]
            del self.npu_context_meta[instance_id]
            logger.info("Unregistered KV cache for NPU ID %d", instance_id)
            torch.npu.empty_cache()
        else:
            logger.warning("No KV cache found for NPU ID %d to unregister", instance_id)
    ...
    def store(
        ...
        assert instance_id in self.npu_contexts, (
            f"KV cache not registered for NPU ID {instance_id}"
        )
        npu_context = self.npu_contexts[instance_id]
        model_name = self.npu_context_meta[instance_id][0]

        blocks_per_chunk = self.chunk_size // npu_context.block_size

        with (
            torch.npu.set_device(npu_context.device),
            torch.npu.stream(npu_context.stream),
        ):
            event = torch.npu.Event(interprocess=True)

            # Stage all block_ids to NPU once before the loop
            all_block_ids_npu = npu_context.stage_block_ids(gpu_block_ids)

            # Wait for vLLM to finish
            vllm_event = torch.npu.Event.from_ipc_handle(
                npu_context.device, event_ipc_handle
            )
            vllm_event.wait(stream=npu_context.stream)

            # CPU-synchronous sentinel: a NPU store is about to be enqueued.
            ...
            self._event_bus.publish(
                Event(
                    ...
                    metadata={"device": str(npu_context.device)},
                )
            )

            self._event_bus.publish_on_stream(
                npu_context.npu_stream_,
                Event(
                    event_type=EventType.MP_STORE_START,
                    session_id=key.request_id,
                    metadata={
                        "device": str(npu_context.device),
            ...
            reserved_dict: dict = {}
            try:
                layout_desc = get_layout_desc(npu_context, self.chunk_size)
                ...
                for idx, obj_key in enumerate(obj_keys):
                    ...
                    chunk_block_ids_npu = all_block_ids_npu[
                        idx * blocks_per_chunk : (idx + 1) * blocks_per_chunk
                    ]

                    # Copy from NPU paged buffer to tmp buffer, then to CPU — per group
                    for group_idx in range(num_groups):
                        tmp_buffer = npu_context.get_tmp_chunk_gpu_buffer(group_idx)
                        group_kv_pointers = npu_context.get_group_kv_pointers(group_idx)
                        lmc_ops.multi_layer_block_kv_transfer(
                            ...
                            npu_context.get_shape_desc(group_idx),
                            self.chunk_size,
                            npu_context.gpu_kv_format_,
                            0,
                        )
                    # Store is not batched, so we always use chunk_idx=0 (single slot)
                    lmcache_memcpy_async_d2h(
                        npu_context.get_tmp_gpu_buffer_flat(chunk_idx=0), memory_obj
                    )
            except Exception:
                ...
                event.record()
                if reserved_dict:
                    npu_context.npu_stream_.launch_host_func(
    ...
    def retrieve(
        ...
        """
        Retrieves the CPU KV cache and put into NPU blocks.

        Args:
            key (IPCCacheEngineKey): The IPC key for the KV cache blocks.
                Must have worker_id != None (worker retrieve operation).
            instance_id (int): The NPU instance ID (such as PID).
            gpu_block_ids (list[int]): The NPU block IDs to retrieve into.
            event_ipc_handle (bytes): The IPC handle of the event to wait on.
            skip_first_n_tokens (int): Number of tokens to skip writing at
                the start of the retrieve range. This avoids overwriting
                APC-shared NPU blocks that may be read concurrently by other
                requests.
        ...
        # CPU-synchronous sentinel: a NPU retrieve is about to be enqueued.
        # Must be published via publish() (not publish_on_stream) so the
        ...
        def _retrieve_loop(keys: list[ObjectKey], memory_objs: list[MemoryObj]) -> None:
            _BATCH_SIZE = npu_context.max_batch_size
            num_groups = npu_context.kv_layer_groups_manager.num_groups
            for batch_idx, memory_obj_batch in enumerate(
                ...
                if skip_tokens_in_chunk % npu_context.block_size != 0:
                    logger.error(
                        "skip_first_n_tokens (%d) is not aligned to block_size (%d), "
                        "rounding down from %d tokens to %d blocks",
                        skip_first_n_tokens,
                        npu_context.block_size,
                        skip_tokens_in_chunk,
                        skip_tokens_in_chunk // npu_context.block_size,
                    )
                skip_blocks_in_chunk = skip_tokens_in_chunk // npu_context.block_size
                ...
                # Copy from CPU to NPU tmp buffers, then scatter to paged KV — per group
                # H2D copy: each memory_obj maps to its own batch slot
                for chunk_idx, memory_obj in enumerate(memory_obj_batch):
                    lmcache_memcpy_async_h2d(
                        memory_obj,
                        npu_context.get_tmp_gpu_buffer_flat(chunk_idx=chunk_idx),
                    )
                for group_idx in range(num_groups):
                    tmp_buffers = npu_context.get_tmp_chunk_gpu_buffer_batched(
                        batch_len, group_idx
                    )
                    group_kv_pointers = npu_context.get_group_kv_pointers(group_idx)
    ...
    def lookup(
        ...
        if layout_desc is None:
            logger.error(
                "No NPU context found for model %s with world size %d during lookup!",
                model_name,
                world_size,
            )
    ...
    def report_status(self) -> dict:
        ...
        npu_context_meta: dict[str, dict] = {}
        for npu_id, meta in self.npu_context_meta.items():
            entry: dict = {
                "model_name": meta[0],
                "world_size": meta[1],
            }
            ctx = self.npu_contexts.get(npu_id)
            if ctx is not None:
            ...
            npu_context_meta[str(npu_id)] = entry
        ...
        return {
            ...
            "registered_npu_ids": list(self.npu_contexts.keys()),
            "npu_context_meta": npu_context_meta,
    ...
    def close(self) -> None:
        """
        Closes the NPUCacheEngine and releases all resources.
        """
        # Close storage manager
        self.storage_manager.close()
        logger.info("NPUCacheEngine closed")

        # Release NPU contexts
        self.npu_contexts.clear()
...
def run_cache_server(
    ...
    """
    Run the LMCache-Ascend cache server with ZMQ message queue.
    ...
    Returns:
        If return_engine is True: tuple of (MessageQueueServer, NPUCacheEngine)
    ...
    logger.info(
        "LMCache-Ascend ZMQ cache server is running on tcp://%s:%d",
        mp_config.host,
        mp_config.port,
    )
    ...
    logger.info("LMCache-Ascend cache server is running...")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LMCache-Ascend ZMQ Cache Server (without HTTP)"
    )
