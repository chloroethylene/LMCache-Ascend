...
from lmcache_ascend.v1.multiprocess.futures import (
    MessagingFuture,
)
...
_SPECIAL_ENCODER_DECODERS = {
    AscendIPCWrapper: (
        get_customized_encoder(AscendIPCWrapper),
        get_customized_decoder(AscendIPCWrapper),
    ),
    list[AscendIPCWrapper]: (
        get_customized_encoder(list[AscendIPCWrapper]),
        get_customized_decoder(list[AscendIPCWrapper]),
    ),
}
...
class MessageQueueServer:
    ...
    def add_normal_thread_pool(
        ...
        Use this for non-NPU blocking handlers (e.g. LOOKUP, END_SESSION).
    ...
    def add_affinity_thread_pool(
        ...
        Use this for NPU-bound blocking handlers (e.g. STORE, RETRIEVE).
